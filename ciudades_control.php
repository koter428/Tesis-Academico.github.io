...
if(strcmp($accion,"1") == 0 || strcmp($accion,"2") == 0){ 
    $sql = "select sp_ciudades(" . $accion . "," . (!empty($_REQUEST['id_ciudad']) ? $_REQUEST['id_ciudad'] : 0) . ",'" .
    (!empty($_REQUEST['nombre']) ? $_REQUEST['nombre'] : '') . "'," .
    (!empty($_REQUEST['id_dpto']) ? $_REQUEST['id_dpto'] :  1) . ") as resul";
}
else if (strcmp($accion,"3") == 0){
    $sql = "select sp_ciudades(" . $accion . "," . $_REQUEST['id_barrio']. ") as resul";
}
...
if (isset($mensaje)) {
    ...
    header("location:" . "ciudades_index.php");
}