...
if (!empty(isset($_REQUEST['opcion']))) {
    switch ($_REQUEST['opcion']) {
        ...
        case 3: //articulo
            ...
            break;
        case 4: //empleado
            $cabeceras = consultas::get_datos("select * from v_pedido_cabcompra where id_empleado in(" . $_REQUEST['vempleado'] . ")");
            break;
    }
} else {
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        $pdf->Cell(130, 2, "PROVEEDOR: " . $cabecera['prv_ruc'] . "-" . $cabecera['proveedor'], 0, '', 'L');
...
//COLOR DE TABLA
        ...
        if (!empty($detalles)) {
            ...
            $pdf->Cell(60, 2, number_format($cabeceras[0]['ped_total'], 0, ",", "."), 1, 0, 'R', 1);
            $pdf->Ln();
            $pdf->Ln();
            $pdf->SetFont('times', '', 11);
        } else {
            $pdf->Cell(165, 2, "El pedido de compra no posee detalles", 0, '', 'L', 1);
            $pdf->Ln();
            $pdf->Ln();
            $pdf->SetFont('times', '', 11);
        }
